...
def print_logo():
    ...
    console.print('[bold green]       Wi-Fi Scanner & Attack Tool by Abz[/bold green]')
    console.print('[bold green]' + '=' * 57 + '[/bold green]\n')
...
def _center_block(lines, styles=None, extra_lines=None):
    ...
    max_w = max([len(ln) for ln in lines] + [len(t) for t, _ in extra_lines] + [0])
...
def intro_animation():
    ...
    for ch in SUBTITLE:
        typed += ch
        clear()
        _center_block(lines, extra_lines=[(typed, 'bold green')])
        time.sleep(0.02)
    for style in ('bold bright_white', 'bold white', 'bold bright_cyan'):
        clear()
        _center_block(lines, styles=style, extra_lines=[(SUBTITLE, 'bold green')])
        time.sleep(0.14)
    clear()
    _center_block(lines, extra_lines=[(SUBTITLE, 'bold green')])
    time.sleep(0.55)
    clear()
...
def _merge_network(bssid, ssid, channel, signal, crypto):
    ...
    if prev and prev.get('signal', -999) > sig:
        ...
        if str(ssid).startswith('<'):
            ssid = prev['ssid']
    networks[bssid] = {'ssid': ssid, 'channel': ch_int, 'signal': sig, 'crypto': str(crypto or '?'), 'band': band}

def results_table():
    t = Table(show_header=True, header_style='bold white', expand=True, box=box.SIMPLE_HEAD, pad_edge=False, show_edge=False, padding=(0, 1))
    ...
    for idx, (bssid, info) in enumerate(sorted(networks.items(), key=lambda x: x[1]['signal'], reverse=True), 1):
        s = info['signal']
        sc = 'green' if s >= -60 else 'yellow' if s >= -75 else 'red'
        t.add_row(str(idx), str(info['ssid'])[:22], bssid, str(info['channel']), info['band'], f'[{sc}]{s}[/{sc}]', str(info['crypto'])[:14])
    return t

def live_table(time_left, mode='scan'):
    t = Table(title=f'Skaner · {len(networks)} ta · {time_left}s', show_header=True, header_style='bold cyan', expand=True, show_edge=False, box=box.SIMPLE)
    ...
    for idx, (bssid, info) in enumerate(sorted(networks.items(), key=lambda x: x[1]['signal'], reverse=True), 1):
        t.add_row(str(idx), str(info['ssid'])[:18], bssid, str(info['channel']), str(info['signal']))
    return t
...
def scan_airodump(mon_iface: str, seconds: int=30) -> int:
    ...
    dump_cmd = f'airodump-ng --write {prefix} --output-format csv --write-interval 1 --band abg {mon_iface}'
    page('Skaner')
    ...
    term_proc = wu.open_side_terminal(title=f'airodump · {mon_iface}', command=dump_cmd, log=_log, geometry='105x34-16+48')
...
def _scan_airodump_inline(mon_iface, prefix, csv_path, seconds) -> int:
    ...
    try:
        proc = subprocess.Popen(['airodump-ng', '--write', prefix, '--output-format', 'csv', '--write-interval', '1', '--band', 'abg', mon_iface], stdout=subprocess.DEVNULL, stderr=err_f)
...
def scan_scapy(mon_iface: str, seconds: int=16) -> int:
    ...
    th = threading.Thread(target=lambda: sniff(iface=mon_iface, prn=on_pkt, timeout=seconds, store=False), daemon=True)
    th.start()
    t0 = time.time()
    try:
...
def show_results_and_pick(mon_iface):
    ...
    console.print()
    console.print(f'  [bright_black]Jami: {len(networks)} ta  ·  raqam = tanlash  ·  r = qayta  ·  0 = chiqish[/bright_black]')
...
def target_menu(bssid, info, mon_iface):
    ...
    console.print(f"  Kanal        [green]{info['channel']}[/green]  [blue]{info['band']}[/blue]")
...
def run_evil_twin(bssid, info, mon_iface):
    ...
    try:
        result = twin.run(mon_iface=mon_iface, essid=essid, bssid=bssid, channel=ap_ch, deauth_channel=real_ch, timeout=0, continuous_deauth=True, ap_essid=essid)
    except KeyboardInterrupt:
        twin.stop()
