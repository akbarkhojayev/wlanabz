...
def check_root():
    ...
    if os.geteuid() != 0:
        ...
        print(
            "    Sudo bilan ishga tushiring: sudo"
...
def set_interface_channel(monitor_iface, target_channel):
    ...
    try:
        ...
        console.print(
            "[bold red][-] Kanalni sozlashda xatolik"
            f" ({target_channel}): {e}[/bold red]"
        )
        return False
...
def enable_monitor_mode():
    ...
    try:
        ...
        current_monitor_iface = monitor_iface
        console.print(
...
def packet_handler(pkt):
    ...
    if pkt.haslayer(Dot11Beacon):
        ...
        freq_band = (
            "5 GHz" if isinstance(channel, int) and channel > 14 else "2.4 GHz"
        )
...
def scan_wifi_networks(monitor_iface):
    ...
    try:
        ...
        console.print(
            "\n[bold yellow][!] Skanerlash to'xtatildi.[/bold yellow]"
        )
...
def show_selected_target_menu(target_bssid, target_info, monitor_iface):
    ...
    action_panel = Panel(
        "  [bold yellow]1[/bold yellow] -> Tarmoq xavfsizligini audit qilish"
        " (WPA3 / PMF Check)\n  [bold green]2[/bold green] -> Bosh menyuga"
    ...
    if choice == "1":
        console.print(
            "\n[bold cyan][*] Tarmoq xavfsizligi va PMF (802.11w) tahlil"
            " qilinmoqda...[/bold cyan]"
        )
        time.sleep(1)
        ...
        if isinstance(target_channel, int):
            console.print(
                f"[bold cyan][*] Wi-Fi kartasi {target_channel}-kanalga"
                " o'tkazilmoqda...[/bold cyan]"
            )
...
def restore_network(monitor_iface):
    ...
    if monitor_iface:
        ...
        console.print(
            "[bold green][+][/bold green] Internet va Wi-Fi holati qayta"
            " tiklandi."
        )
...
def main():
    ...
    welcome_panel = Panel(
        ...
        " Wi-Fi skanerlashni boshlash (2.4GHz + 5GHz)\n  [bold red]0[/bold red]"
        " -> Dasturdan chiqish",
...
if __name__ == "__main__":
    try:
        ...
        console.print(
            "\n\n[bold yellow][!] Dastur foydalanuvchi tomonidan"
            " to'xtatildi (Ctrl+C).[/bold yellow]"
        )
        if current_monitor_iface:
            console.print(
                "[bold cyan][*] Internet ulanishi qayta"
                " tiklanmoqda...[/bold cyan]"
            )
            restore_network(current_monitor_iface)
        sys.exit(0)
